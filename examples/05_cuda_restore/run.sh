...
cleanup() {
    ...
    sudo rm -rf "$parent"
    rm -f examples/05_cuda_restore/cuda-fixture
}
trap cleanup EXIT INT TERM

cd "$repo_root"
gcc -I/usr/local/cuda/include examples/05_cuda_restore/cuda-fixture.c \
    -L/usr/local/cuda/lib64 -Wl,-rpath,/usr/local/cuda/lib64 -lcuda \
    -o examples/05_cuda_restore/cuda-fixture

./target/debug/edo run --name "$name" -- setsid \
    examples/05_cuda_restore/cuda-fixture >"$launch_log" 2>&1
