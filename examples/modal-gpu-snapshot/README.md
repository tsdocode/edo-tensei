...
latency before/after. The fixture also writes a startup marker and verifies that
model initialization occurred exactly once across the restore.
...
restore time. It is intentionally single-process and single-GPU; in-flight
requests, multi-process serving workers, CUDA graphs, KV-cache portability,
and cross-GPU migration are not covered by this demo.
